...
object GasVectorGenerator extends IOApp {
  ...
  private val Corpus: List[CategorySpec] = List(
    ...
    CategorySpec(
      "control_flow",
      Some(
        "The tail-recursive runtime evaluates if/let lazily and charges NO base cost for them " +
        "(normative: see metakit PR #37); only the condition / bindings / taken branch pay."
      ),
      List(
        CaseSpec("{\"if\": [true, 1, 2]}", "{}", Abs(1000), Some("constant condition + constant branch: 0 gas")),
        CaseSpec(
          "{\"if\": [{\">\": [5, 3]}, {\"+\": [1, 2]}, 99]}",
          "{}",
          Abs(1000),
        ...
        CaseSpec("{\"let\": [{\"a\": 1}, {\"var\": \"a\"}]}", "{}", Abs(1000), Some("constant binding: only the var lookup pays")),
        CaseSpec(
          "{\"let\": [{\"a\": {\"+\": [1, 2]}}, {\"+\": [{\"var\": \"a\"}, 1]}]}",
          "{}",
          Abs(1000),
          Some("binding expression and result expression both pay; let itself does not")
        )
      )
    ),
    CategorySpec(
        ...
        JsonObject.fromIterable(
          List(
            "description" := (
              ...
              "consume varAccess + #pathSegments at lookup; if/let charge no base cost; gasUsed is the " +
              "gas-counter delta. Generated by `sbt \"Test/runMain json_logic.GasVectorGenerator\"` — " +
