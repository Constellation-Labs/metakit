...
trait JsonCanonicalizer[F[_]] {
  def canonicalize[A: Encoder](content: A): F[Json]
}
...
object JsonCanonicalizer {
  ...
  def make[F[_]: MonadThrow]: JsonCanonicalizer[F] = new JsonCanonicalizer[F] {
    private def escape(c: Char): String = s"\\$c"

    private def serializeString(value: String): String = {
      val buffer = new StringBuilder(value.length + 2)
      buffer.append('"')
      value.foreach {
        case '\n' => buffer.append(escape('n'))
        case '\b' => buffer.append(escape('b'))
        case '\f' => buffer.append(escape('f'))
        case '\r' => buffer.append(escape('r'))
        case '\t' => buffer.append(escape('t'))
        case c @ ('"' | '\\') => buffer.append(escape(c))
        case c if c < 0x20 =>
          buffer.append("\\u")
          var hex = c
          for (_ <- 1 to 4) {
            val digit = (hex >>> 12) & 0xF
            buffer.append((if (digit > 9) digit - 10 + 'a' else digit + '0').toChar)
            hex = (hex << 4).toChar
          }
    ...
    private def encode(json: Json): F[String] = {
      def loop(json: Json): F[String] = json.fold(
        "null".pure[F],
        bool => bool.toString.pure[F],
        num => NumberToJson.serializeNumber(num.toDouble),
        str => serializeString(str).pure[F],
        arr => arr.traverse(loop).map(values => s"[${values.mkString(",")}]"),
        obj => {
          val sortedFields = TreeMap.from(obj.toMap)
          sortedFields.toList.traverse { case (key, value) =>
            loop(value).map(v => s"${serializeString(key)}:$v")
          }.map(entries => s"{${entries.mkString(",")}}")
        }
      )
      loop(json)
    }

    def canonicalize(json: Json): F[Json] =
      encode(json).flatMap(s => MonadThrow[F].fromEither(parse(s)))

    def canonicalize(jsonData: String): F[String] =
      MonadThrow[F].fromEither(parse(jsonData)).flatMap(encode)

    def canonicalize(jsonData: Array[Byte]): F[Json] =
      canonicalize(new String(jsonData, StandardCharsets.UTF_8)).map(_.asJson)

    def getEncodedUTF8(jsonData: String): F[Array[Byte]] =
      canonicalize(jsonData).map(_.getBytes(StandardCharsets.UTF_8))

    def canonicalize[A: Encoder](content: A): F[Json] =
      canonicalize(content.asJson)
  }

  implicit class JsonPrinterEncodeOps[F[_], A](private val _v: A) extends AnyVal {
    def asCanonicalJson(implicit mt: MonadThrow[F], enc: Encoder[A]): F[Json] =
...
private class DoubleSerializerImpl[F[_]: MonadThrow] {
  ...
  def serialize(value: Double, ecmaMode: Boolean): F[String] = {
    ...
    val components = FloatComponents(
      e2 = if (ieeeExponent == 0) 1 - DoubleExponentBias - DoubleMantissaBits
      else ieeeExponent - DoubleExponentBias - DoubleMantissaBits,
      m2 = if (ieeeExponent == 0) ieeeMantissa
    ...
    def computeDecimalBase(c: FloatComponents): F[DecimalBase] = {
      ...
      if (e2Adj >= 0) {
        ...
        for {
          ...
          dmz <- if (q <= 21 && c.even) multipleOfPowerOf5(c.mm, q)
          else false.pure[F]
...
private object DoubleSerializerImpl {
  ...
  private val (pow5Split, pow5InvSplit) = {
    ...
    val results = (0 until math.max(PosTableSize, NegTableSize))
      .toVector
      .map(processIndex)
