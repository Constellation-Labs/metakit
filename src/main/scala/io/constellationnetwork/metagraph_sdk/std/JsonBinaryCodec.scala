    ...
    new JsonBinaryCodec[F, A] {
      ...
      def serialize(content: A): F[Array[Byte]] =
        for {
          ...
          bytes <- json.getBytes("UTF-8").pure[F]
        } yield bytes
  ...
  implicit def deriveDataUpdate[F[_]: MonadThrow, U <: DataUpdate: Encoder: Decoder]: JsonBinaryCodec[F, U] =
    new JsonBinaryCodec[F, U] {
      ...
      def serialize(content: U): F[Array[Byte]] =
        for {
          json         <- content.asCanonicalJson.map(_.noSpaces)
          jsonBytes    <- json.getBytes("UTF-8").pure[F]
          base64String <- Base64.toBase64String(jsonBytes).pure[F]
      ...
      def deserialize(bytes: Array[Byte]): F[Either[Throwable, U]] =
        for {
          ...
          jsonBytes    <- Base64.decode(base64String).pure[F]
          jsonStr      <- new String(jsonBytes, "UTF-8").pure[F]
